...
export function buildProductSystemPrompt(): string {
  ...
  return [
    ...
    '',
    'Правило выбора: соседний уровень → `team.invite`, через уровень → `team.escalate`. Самому собирать цепочку посредников вручную не надо — это и есть работа `team.escalate`.',
    '',
    "Для тебя это означает: если нужен прямой ответ программиста (через уровень — между вами архитектор), вызывай `team.escalate('programmer', '<вопрос>')` — архитектор окажется в комнате автоматически. Архитектора зови через `team.invite('architect', '<сообщение>')` — это сосед.",
    '',
